...
public class KafkaConsumerService {
	...
	@KafkaListener(topics = KafkaConstants.KAFKA_CHAT_TOPIC, groupId = KafkaConstants.GROUP_ID)
	public void receiveChat(MessageDto message) throws IOException {
	...
	@KafkaListener(topics = KafkaConstants.KAFKA_NOTICE_TOPIC, groupId = KafkaConstants.GROUP_ID)
	public void receiveNotice(NoticeDto notice) throws IOException {
		...
		log.info("전송 위치 = /sub/notice/" + notice.getRoomId());
		// log.info("채팅 방으로 메시지 전송 = {}", message.toString());
